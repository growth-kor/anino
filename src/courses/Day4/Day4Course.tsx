...
import { Day4Simulator } from './Day4Simulator';
import { CodeWorkspace, type LogicBlock } from '../../components/editor/CodeWorkspace';
import { Day4_Steps } from './Day4Content';
import confetti from 'canvas-confetti';

interface Day4CourseProps {
  onBackToLanding: () => void;
}

export const Day4Course: React.FC<Day4CourseProps> = ({ onBackToLanding }) => {
  ...
  const step = Day4_Steps[currentStepIndex];
  ...
  const handleNextStep = () => {
    if (currentStepIndex < Day4_Steps.length - 1) {
    ...
    <div className="app-container" style={{ fontSize: '1.05rem' }}>
      ...
      <header className="app-header" style={{ position: 'relative', zIndex: 50, padding: '16px 32px' }}>
        ...
        <div className="flex-center" style={{ gap: '16px' }}>
          <div style={{ position: 'relative' }}>
            ...
            >
              <BookOpen size={18} />
              진도 요약 ({currentStepIndex + 1}/{Day4_Steps.length})
              <ChevronDown size={16} />
            </button>
      ...
      <div className="workspace-layout" style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {step.type === 'theory' ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: 'var(--color-bg-base)', padding: '40px 30px', overflowY: 'auto' }}>
            <div className="glass-panel animate-fade-in" style={{ maxWidth: '1000px', width: '100%', padding: '50px 60px', borderRadius: 'var(--radius-2xl)', border: '1px solid var(--color-glass-border)', boxShadow: '0 20px 50px rgba(0,0,0,0.5)', background: 'var(--color-bg-surface-elevated)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '20px' }}>
                ...
                {currentStepIndex < Day4_Steps.length - 1 ? (
                  <button 
          ...
          /* Practice View containing code workspace + custom Day 4 simulator */
          <>
            <div style={{ flex: 1.1, borderRight: '1px solid var(--color-glass-border)', display: 'flex', flexDirection: 'column', padding: 'var(--spacing-4)', background: 'var(--color-bg-base)' }}>
              <CodeWorkspace blocks={codeBlocks} setBlocks={setCodeBlocks} day={4} />
            </div>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  ...
                  >
                    {isCurrentStepCleared()
                      ? (currentStepIndex === Day4_Steps.length - 1 ? '🎉 Day 4 완료!' : '미션 클리어! 다음 ➔') 
                      : '미션을 완료해야 다음으로 넘어갑니다'}
                  </button>
                </div>
              </div>
